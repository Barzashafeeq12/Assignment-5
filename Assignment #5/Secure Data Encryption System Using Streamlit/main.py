...
def decrypt_data(encrypted_text, passkey):
    ...
    failed_attempts += 1  # Increment failed attempts on incorrect passkey
    return None
        ...
        if encrypted_text and passkey:
            ...
            if decrypted_text:
                ...
                if check_failed_attempts():
                    ...
                    st.experimental_rerun()  # Redirect to login page after too many failed attempts
        else:
            st.error("⚠️ Both fields are required!")
...
if st.button("Login"):
        if login_pass == "admin123":  # Hardcoded for demo, replace with proper auth
            globalfailed_attempts = 0  # Reset the failed attempts on successful login
